...
$(document).ready(function () {
    ...
    // Handle save quick note button
    $("#save-quick-note").on("click", function () {
        const bookingId = $(this).data("booking-id");
        const noteText = $("#quick-note-text").val().trim();
        ...
        saveQuickNote(bookingId, noteText);
    });
    ...
    } // Function to load booking notes
    function loadBookingNotes(bookingId) {
        ...
        // Fetch notes via API
        $.ajax({
            url: `/api/bookings/${bookingId}/notes`,
            method: "GET",
            success: function (response) {
                if (
                    ...
                    response.notes.forEach((note) => {
                        const noteDate = moment(note.created_at).format(
                            "MMM D, YYYY • h:mm A"
                        );

                        notesHtml += `
                            <div class="note-item card bg-lighter p-2 mb-2">
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="fw-semibold">${note.staff}</span>
                    ...
                    $("#view-quick-notes").html(
                        '<p class="text-muted fst-italic">No notes added yet.</p>'
                    );
                }
            },
            error: function (xhr) {
                console.error("Error loading booking notes:", xhr);
                $("#view-quick-notes").html(
                    '<p class="text-danger">Failed to load notes. Please try again.</p>'
                );
            },
        });
    } // Function to save a quick note
    function saveQuickNote(bookingId, noteText) {
        // Show loading state
        const saveBtn = $("#save-quick-note");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // If we have the newly created note in the response, we can add it directly
                if (response.success && response.note) {
                    // Format the note and add it to the top of the list
                    const noteDate = moment(response.note.created_at).format(
                        "MMM D, YYYY • h:mm A"
                    );
                    const staffName = response.note.user
                        ? response.note.user.name
                        : "Current User";

                    const newNoteHtml = `
                        <div class="note-item card bg-lighter p-2 mb-2" style="display:none;">
                            <div class="d-flex justify-content-between mb-1">
                                <span class="fw-semibold">${staffName}</span>
                                <small class="text-muted">${noteDate}</small>
                            </div>
                            <p class="mb-0">${response.note.note}</p>
                        </div>
                    `;

                    // Check if we already have notes or if this is the first one
                    if (
                        $("#view-quick-notes").find(".note-item").length === 0
                    ) {
                        $("#view-quick-notes").html(newNoteHtml);
                    } else {
                        $("#view-quick-notes").prepend(newNoteHtml);
                    }

                    // Show the new note with a highlight effect
                    $("#view-quick-notes")
                        .find(".note-item:first")
                        .slideDown()
                    ...
                    // Fallback to reloading all notes
                    loadBookingNotes(bookingId);
                }
            },
            error: function (xhr) {
